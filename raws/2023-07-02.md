...
My articles are named by publication date (ISO format), so they can simply be sorted alphabetically to get them in chronological order. They don't currently hold any special metadata such as [madblog](https://pypi.org/project/madblog/), which I would likely have used if I had not felt like using the opportunity to better learn Nicegui.
